...
const CreateRegistration = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.certifications.forEach((c) => {
        if (c.file) data.append("certificationFiles", c.file);
      });
  ...
  if (initialLoading)
    return (
      <div
        className="p-10 text-center font-bold opacity-50"
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold uppercase tracking-tight">
            Add New Laboratory Registration
          </h1>
        </div>
        <button
          ...
          type="submit"
          disabled={loading}
          className="px-8 py-3 bg-black text-white font-bold text-xs uppercase tracking-widest rounded transition-all hover:opacity-80 disabled:opacity-50"
        >
      ...
      <form id="registration-form" onSubmit={handleSubmit} className="mb-20">
        ...
        >
          ...
          <div className="w-full">
            <label className={labelStyle}>
              Parent Organization (Select if affiliated with a chain)
            </label>
            <div className="w-full">
              <ModernSelect
                fullWidth
                ...
                options={[
                  {
                    label: "-- No Parent (Individual Laboratory) --",
                    value: "",
                  },
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {/* <div>
              <label className={labelStyle}>Staff Count (Professional)</label>
              <input
                type="number"
                name="staffCount"
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-8">
            <div
              ...
              className={`flex items-center gap-3 p-4 border rounded cursor-pointer transition-all ${formData.homeCollection ? "bg-black/5 border-black/40" : "bg-transparent border-black/10 hover:border-black/20"}`}
            >
            ...
            <div
              onClick={() => setFormData((p) => ({ ...p, is24x7: !p.is24x7 }))}
              className={`flex items-center gap-3 p-4 border rounded cursor-pointer transition-all ${formData.is24x7 ? "bg-black/5 border-black/40" : "bg-transparent border-black/10 hover:border-black/20"}`}
            >
              <div className="text-xl">
                {formData.is24x7 ? <MdCheckBox /> : <MdCheckBoxOutlineBlank />}
              </div>
              <span className="text-[11px] font-bold uppercase tracking-tight opacity-80">
                24X7 OPEN
              </span>
            ...
            <div
              ...
              className={`flex items-center gap-3 p-4 border rounded cursor-pointer transition-all ${formData.emergency ? "bg-black/5 border-black/40" : "bg-transparent border-black/10 hover:border-black/20"}`}
            >
              <div className="text-xl">
                {formData.emergency ? (
                  <MdCheckBox />
        ...
        >
          <h2 className="text-xs font-black uppercase mb-4 border-b pb-2 opacity-80">
            Pathology Registration Documents
          </h2>
          <div
            className="border border-dashed p-6 rounded text-center"
            style={{ borderColor: colors.accent + "30" }}
          >
            <input
              type="file"
            ...
            >
              <MdCloudUpload size={32} className="opacity-40" />
              <div className="text-xs font-bold uppercase tracking-widest">
                {formData.pathologyDocs
                  ? formData.pathologyDocs.name
                  : "Select Registration Documents (PDF/Docs)"}
              </div>
              <div className="text-[9px] opacity-50">
                Upload building fitness, fire NOC or clinical establishment
                certificate
              </div>
            </label>
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              <label className={labelStyle}>Weekly Off</label>
              <div className="w-full">
                <ModernSelect
                  fullWidth
                  value={formData.weeklyOff}
                  onChange={(val) =>
                    setFormData((prev) => ({ ...prev, weeklyOff: val }))
                  }
                  options={[
                    { label: "None", value: "" },
        ...
        >
          <button
            type="submit"
            disabled={loading}
            className="px-12 py-4 bg-black text-white font-black text-sm uppercase tracking-[0.2em] rounded shadow-xl transition-all hover:scale-105 active:scale-95 disabled:opacity-50"
          >
